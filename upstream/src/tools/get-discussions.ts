...
async function getForumsOverview(
  ...
  for (const forum of forums) {
    ...
    try {
      ...
      topicsReadError = "Could not read topics; an empty list here does not prove the forum is empty.";
      if (error?.status === 403) {
    ...
    result.push({
      ...
      ...(topicsReadError ? { topicsReadError } : {}),
      topics: topics.map((t) => ({
        topicId: t.TopicId,
        forumId: t.ForumId,
        name: t.Name,
...
async function getForumDetail(
  ...
  for (const topic of topics) {
    ...
    try {
      ...
      postsReadError = "Could not read posts; an empty list here does not prove the topic is empty.";
      if (error?.status === 403) {
